...
vm_int_instr:
    ...
    mov si, word [TEMP_VALUE]
    call decode_operand
    ; AX = interrupt number (8-bit in AL)
    ; Push interrupt number, then call int with immediate
    push ax
    mov al, byte [TEMP_VALUE]
    ; We need to call int with immediate - this is tricky
    ; For now, use a lookup table or dynamic call
    ; TODO: Fix this properly - need to use int with immediate value
    pop ax
    jmp vm_loop_continue
...
bytecode_start:
    ...
    db 0x52, 0x4D, 0x53, 0x4D  ; "RMSM" magic
    ; ... rest of bytecode ...
bytecode_end:
